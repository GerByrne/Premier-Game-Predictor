   ...
   <div class="bluebg" > 
    ...
    <!---------------------------------------------- Navi Bar Game set ----------------------------------------->
		<div id="radio" align="center" style="margin-top:-34px">
		  ...
		  <input type="radio" id="radio3" name="radio" checked="checked" /><label for="radio3"><a href="game3.php">
		  <?php
     ...
     <div id = "fixtureList">
 		<table class = "gameweek" align="center" cellspacing="2" style="padding:30px 4px" >
 			<tr> 				
 				<td>
 				<?php
				$query40 = mysql_query("SELECT filename FROM elo_rating WHERE game_number='3h'");
				...
				<?php
				$query30 = mysql_query("SELECT team_name FROM elo_rating WHERE game_number='3h'");
    			$result30 = mysql_fetch_assoc($query30);
    			$teamA_name = $result30['team_name'];

    			$query31 = mysql_query("SELECT team_name FROM elo_rating WHERE game_number='3a'");
				...
				<td>
 				<?php
				$query40 = mysql_query("SELECT filename FROM elo_rating WHERE game_number='3a'");
    			$result40 = mysql_fetch_assoc($query40);
    			$image = $result40['filename'];    	
				echo "<img src = 'images/icons/$image'>";    			
				?>
...
<?php
	$query1 = mysql_query("SELECT * FROM elo_rating WHERE game_number='3h'");
 	$result1 = mysql_fetch_assoc($query1);
    $rating1 = $result1['ranking']+80;											//adding 80 for the home advantage

	$query2 = mysql_query("SELECT * FROM elo_rating WHERE game_number='3a'");
 	$result2 = mysql_fetch_assoc($query2);
    $rating2 = $result2['ranking'];

    $query3 = mysql_query("SELECT team_name FROM elo_rating WHERE game_number='3h'");
    $result3 = mysql_fetch_assoc($query3);
    $teamA_name = $result3['team_name'];

    $query4 = mysql_query("SELECT team_name FROM elo_rating WHERE game_number='3a'");
    $result4 = mysql_fetch_assoc($query4);
    $teamB_name = $result4['team_name'];

    $query5 = mysql_query("SELECT lastweeks_K FROM elo_rating WHERE game_number='3h'");				//This selects one teams k factor (Fulham)
    $result5 = mysql_fetch_assoc($query5);
    $k_factor = $result5['lastweeks_K'];

    $query6 = mysql_query("SELECT goal_diff FROM elo_rating WHERE game_number='3h'");				//This selects the goal diff from one team. Which is all that is needed
    $result6 = mysql_fetch_assoc($query6);
    $gd = $result6['goal_diff'];

	$query7 = mysql_query("SELECT win_lose_draw FROM elo_rating WHERE game_number='3h'");			//This selects Home teams win result
    $result7 = mysql_fetch_assoc($query7);
    $result_A = $result7['win_lose_draw'];

	$query8 = mysql_query("SELECT win_lose_draw FROM elo_rating WHERE game_number='3a'");			//This selects Away teams win result
...
function fulVars()
{
	$query31 = mysql_query("SELECT HomeWins FROM everton WHERE vsTeam='westham'");
    $result31 = mysql_fetch_assoc($query31);
    $homeWinsHtoH = $result31['HomeWins'];
    
	$query32 = mysql_query("SELECT ELORating FROM teamformlast10 WHERE Team = 'westham'");
	$result32 = mysql_fetch_assoc($query32);
	$awayELO = $result32['ELORating'];
    
	$query33 = mysql_query("SELECT TotalMatches FROM everton WHERE vsTeam = 'westham'");
	$result33 = mysql_fetch_assoc($query33);
	$totalHtoHMatches = $result33['TotalMatches'];
    
	$query34 = mysql_query("SELECT HomeWins FROM everton WHERE vsTeam = 'westham'");
 	$result34 = mysql_fetch_assoc($query34);
 	$homeTeamWins  = $result34['HomeWins'];
    
	$query35 = mysql_query("SELECT HomeDefeats FROM everton WHERE vsTeam = 'westham'");
	$result35 = mysql_fetch_assoc($query35);
	$awayTeamWins = $result35['HomeDefeats'];
    
	$query36 = mysql_query("SELECT TotalHomeGames FROM teamformlast10 WHERE Team = 'everton'");
 	$result36 = mysql_fetch_assoc($query36);
 	$homeTeamHomeMatches = $result36['TotalHomeGames'];
    
	$query37 = mysql_query("SELECT HomeGamesWon FROM teamformlast10 WHERE Team = 'everton'");
 	$result37 = mysql_fetch_assoc($query37);
 	$homeTeamHomeWins = $result37['HomeGamesWon'];
   
	$query38 = mysql_query("SELECT TotalAwayGames FROM teamformlast10 WHERE Team = 'westham'");
 	$result38 = mysql_fetch_assoc($query38);
 	$awayTeamAwayMatches = $result38['TotalAwayGames'];
    
	$query39 = mysql_query("SELECT AwayGamesWon FROM teamformlast10 WHERE Team = 'westham'");
